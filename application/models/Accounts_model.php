...
class Accounts_model extends CI_Model
{
    ...
    {
        // $this->db->select('mp_customer_payments.description, mp_customer_payments.id, mp_customer_payments.amount, mp_customer_payments.method, mp_category.category_name, mp_customer_payments.date, mp_customer_payments.customer_id');
        $this->db->select('*');
        $this->db->from($tablename);
        // $this->db->join('mp_category', 'mp_category.id = mp_customer_payments.id_kategori');
    ...
    public function fetch_record_datete($tablename, $first_date, $second_date, $customer_id = null)
    {
        $this->db->select('mp_customer_payments.description, mp_customer_payments.id, mp_customer_payments.amount, mp_customer_payments.method, mp_category.category_name, mp_customer_payments.date, mp_customer_payments.customer_id');
        $this->db->from($tablename);
        $this->db->join('mp_category', 'mp_category.id = mp_customer_payments.id_kategori');
        ...
        //$this->db->order_by('id', 'DSC');
        $this->db->order_by("mp_customer_payments.id", "desc");
        $query = $this->db->get();
        if ($query->num_rows() > 0) {
    ...
    public function Statistics_sales_this_year()
    {
        ...
        for ($counteri = 1; $counteri <= 12; $counteri++) {
            ...
            $result_invoice = $this->fetch_record_date('mp_customer_payments', $date1, $date2);
            if ($result_invoice != null) {
                $count = 0;
                foreach ($result_invoice as $obj_result_invoice) {
