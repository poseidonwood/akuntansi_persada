...
ColVis.prototype = {
	...
	/**
	 * Create a button which allows a "restore" Actions
	 ...
	 *  @private
	 */
	"_fnDomShowXButton": function ( str, Actions )
	{
		var
			that = this,
			dt = this.s.dt;
		...
		return $(
			...
			.click( function (e) {
				for ( var i=0, iLen=that.s.abOriginal.length ; i<iLen ; i++ )
				{
					...
					{
						that.s.dt.oInstance.fnSetColumnVis( i, Actions, false );
					}
				}
				that._fnAdjustOpenRows();
				that.s.dt.oInstance.fnAdjustColumnSizing( false );
	...
	"_fnDomBackground": function ()
	{
		...
		/* When considering a mouse over Actions for the activation, we also consider a mouse out
